...
const float BAYER_2X2[][2] = {{0, 2}, {3, 1}};
...
unsigned char* dither(unsigned char *img, int w, int h, int channels, uint8_t palette[][3],
    ...
    int new_pixel[3]; float bayer_value;
    for(int y = 0; y < h; y++) {
        for(int x = 0; x < w; x++) {
            unsigned char *src_pixel = img + (y * w + x) * 4;
            if (s == 2) { bayer_value = BAYER_2X2[y%2][x%2]; }
            else if (s == 4) { bayer_value = BAYER_4X4[y%4][x%4]; }
            else if (s == 8) { bayer_value = BAYER_8X8[y%8][x%8]; }
            else if (s == 16) { bayer_value = BAYER_16X16[y%16][x%16]; }
            else { s = 32; bayer_value = BAYER_32X32[y%32][x%32]; }
            for(int i = 0; i < 3; i++) {
                new_pixel[i] = src_pixel[i]+255*strength*(bayer_value/(s*s)-0.5);
            }
