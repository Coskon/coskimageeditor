...
unsigned char* dither(unsigned char *img, int w, int h, int channels, uint8_t palette[][3], int plt_size,
    ...
    for(int y = 0; y < h-2; y++) {
        for(int x = 1; x < w-2; x++) {
            ...
            unsigned char *src_pixel4 = img + ((y+1) * w + x) * 4;
            unsigned char *src_pixel5 = img + ((y+1) * w + (x+1)) * 4;
            unsigned char *src_pixel6 = img + ((y+2) * w + x) * 4;
            for (int n=0; n<3; n++) {
            ...
            for (int i=0; i<3; i++) {
                ...
                src_pixel1[i] = clampTo8Bit(src_pixel1[i]+error/8);
                src_pixel2[i] = clampTo8Bit(src_pixel2[i]+error/8);
                src_pixel3[i] = clampTo8Bit(src_pixel3[i]+error/8);
                src_pixel4[i] = clampTo8Bit(src_pixel4[i]+error/8);
                src_pixel5[i] = clampTo8Bit(src_pixel5[i]+error/8);
                src_pixel6[i] = clampTo8Bit(src_pixel6[i]+error/8);
            }
        }
    }
